...
extern "C"
{
	...
	extern void(*const extra_hashes_fedg[8])(const void *, uint32_t, char *);
}
...
#define CN_FINALIZE(n) \
	...
	if(ALGO == cryptonight_fedg) \
	extra_hashes_fedg[ctx[n]->hash_state[0] & 7](ctx[n]->hash_state, 200, (char*)output + 32 * n); \
	else \
...
struct Cryptonight_hash_asm<1, asm_version>
{
	...
	static void hash(const void* input, size_t len, void* output, cryptonight_ctx** ctx)
	{
		...
		if(ALGO == cryptonight_fedg) 
			extra_hashes_fedg[ctx[0]->hash_state[0] & 7](ctx[0]->hash_state, 200, (char*)output);
		else 
...
struct Cryptonight_hash_asm<2, 0>
{
	...
	static void hash(const void* input, size_t len, void* output, cryptonight_ctx** ctx)
	{
		...
		for(size_t i = 0; i < N; ++i)
		{
			...
			if(ALGO == cryptonight_fedg) 
				extra_hashes_fedg[ctx[i]->hash_state[0] & 7](ctx[i]->hash_state, 200, (char*)output + 32 * i);
			else 
